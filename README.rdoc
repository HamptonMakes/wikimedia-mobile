...
3) Setup host headers for en.m.wikipedia.org, *.m.wikipedia.org for any language that you want to test. Honestly, anything works, but it needs to start with a language code. So, you could use ja.wikitest as a host. Point each one at 127.0.0.1 assuming you don't have some sort of freaky setup.
...
7) Browse to http://en.m.wikipedia.org:4000
